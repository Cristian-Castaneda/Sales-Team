// lib/workspace.ts — Shared workspace utilities for the marketing-genius skill
...
/** Generate a simple timestamp-based campaign ID. */
export function generateCampaignId(): string {
  return `camp_${new Date().toISOString().replace(/[-:T]/g, "").slice(0, 14)}`;
}
...
/** Find the latest campaigns.md in workspace/marketing/campaigns/. */
export function findLatestCampaign(campaignId?: string): string | null {
  const base = join(getWorkspaceRoot(), "marketing", "campaigns");
