// lib/workspace.ts — Shared workspace utilities for the image-builder skill
...
/** Get the image output directory for a job. Returns absolute path. */
export function getImageDir(jobId: string): string {
  const dir = join(getWorkspaceRoot(), "assets", "images", getToday(), jobId);
  ensureDir(dir);
  return dir;
}
